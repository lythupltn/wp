...
namespace Lwwb\Modules\Menu;
/************
 * Bulma_NavWalker
 * Author: Tracy Ridge
 * Author URL: https://www.lwwb.co.uk/
...
class Bulma_Navwalker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$attributes  = $this->get_item_attributes( $item, $args, $depth );
	...
	private function get_item_attributes( $item, $args, $depth ) {
		$atts = array();
		...
		$atts           = apply_filters( 'nav_menu_link_attributes', $atts, $item, $depth, $args );
		$attributes     = '';
	...
	public static function fallback( $args ) {
		...
		if ( current_user_can( 'edit_theme_options' ) ) {
			...
			echo '>';
			echo '<a class="navbar-item" href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="">' . esc_attr( 'Add a menu', 'lwwb' ) . '</a>';
			echo '</div>';
			if ( $container ) {
				echo '</' . esc_attr( $container ) . '>';
			}
