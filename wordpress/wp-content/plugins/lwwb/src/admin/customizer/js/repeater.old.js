var RepeaterRow = function (rowIndex, container, label, control) {
    ...
    this.visibility = this.container.find('.visibility');
    this.disableInput = this.container.find('.repeater-field-disable input');
    this.disableInputValue = this.container.find('input.hidden-row').val();

    this.header.on('click', function () {
        self.toggleMinimize();
    });

    if (this.disableInputValue === 'true') {
        this.visibility.find('i').removeClass('dashicons-visibility').addClass('dashicons-hidden');
        this.visibility.next().css('background', '#eee');
        this.visibility.css('background', '#eee')
    }

    this.visibility.on('click', function () {
        // Click visibility update checkbox disable .
        let contentRow = $(this).parent().find('.repeater-row-content .hidden-row');
        $(contentRow).trigger('click');
        if ($(this).find('i').hasClass('dashicons-visibility')) {
            $(this).find('i').removeClass('dashicons-visibility').addClass('dashicons-hidden');
            $(this).next().css('background', '#eee');
            $(this).css('background', '#eee');
        } else {
            $(this).find('i').removeClass('dashicons-hidden').addClass('dashicons-visibility');
...
wp.customize.controlConstructor['lwwb-repeater'] = wp.customize.Control.extend({
    ...
    ready: function () {
        ...
        // Init the control.
        control.initLwwbControl();
    },

    initLwwbControl: function () {
        ...
        // The current value set in Control Class (set in Lwwb_Customize_Repeater_Control::to_json() function)
        ...
        this.repeaterTemplate = _.memoize(function () {
            ...
            return function (data) {
                compiled = _.template($('#customize-control-repeater-content-item').first().html(), null, options);
                // compiled = _.template( control.container.find( '#customize-control-repeater-content-item' ).first().html(), null, options );
    ...
    setImageInRepeaterField: function (attachment) {
        ...
        $targetDiv.find('.lwwb-image-attachment').html('<img src="' + attachment.url + '">').hide().slideDown('slow');
    ...
    setFileInRepeaterField: function (attachment) {
        ...
        $targetDiv.find('.lwwb-file-attachment').html('<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>').hide().slideDown('slow');
    ...
    removeImage: function (event) {
        ...
        $targetDiv.find('.lwwb-image-attachment').slideUp('fast', function () {
    ...
    removeFile: function (event) {
        ...
        $targetDiv.find('.lwwb-file-attachment').slideUp('fast', function () {
            jQuery(this).show().html(jQuery(this).data('placeholder'));
        });
        $targetDiv.find('.hidden-field').val('');
        $uploadButton.text($uploadButton.data('label'));
    ...
    addRow: function (data) {
        ...
        var control = this,
            template = control.repeaterTemplate(), // The template for the new row (defined on Lwwb_Customize_Repeater_Control::render_content() ).
