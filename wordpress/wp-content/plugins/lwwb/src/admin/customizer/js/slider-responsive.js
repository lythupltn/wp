wp.customize.controlConstructor['lwwb-slider'] = wp.customize.Control.extend({
	...
	ready: function () {
		...
		var control = this,
			desktop_slider = control.container.find('.lwwb-slider.desktop-slider'),
			desktop_slider_input = desktop_slider.next('.lwwb-slider-input').find('input.desktop-input'),
			tablet_slider = control.container.find('.lwwb-slider.tablet-slider'),
			tablet_slider_input = tablet_slider.next('.lwwb-slider-input').find('input.tablet-input'),
			mobile_slider = control.container.find('.lwwb-slider.mobile-slider'),
			mobile_slider_input = mobile_slider.next('.lwwb-slider-input').find('input.mobile-input'),
			slider_input,
			$this,
			val;
		...
		jQuery('input.desktop-input').on('change keyup paste', function () {
			$this = jQuery(this);
			val = $this.val();
			slider_input = $this.parent().prev('.lwwb-slider.desktop-slider');
		...
		jQuery('input.tablet-input').on('change keyup paste', function () {
			$this = jQuery(this);
			val = $this.val();
			slider_input = $this.parent().prev('.lwwb-slider.tablet-slider');
		...
		jQuery('input.mobile-input').on('change keyup paste', function () {
			$this = jQuery(this);
			val = $this.val();
			slider_input = $this.parent().prev('.lwwb-slider.mobile-slider');

			slider_input.slider('value', val);
		});
		control.container.on('click', '.lwwb-reset-slider', function () {
			$this = jQuery(this);
			val 	= $this.data( 'reset_value' );
			slider_input = $this.closest('.customize-control-lwwb-slider').find('.lwwb-slider');
			slider_input.slider('value', val);
		});
		// Save the values
